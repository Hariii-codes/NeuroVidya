...
export function useTTS() {
  ...
  const speak = useCallback((text: string, config?: Partial<TTSConfig>) => {
    // Always get latest speechSpeed from store
    const { speechSpeed } = usePreferenceStore.getState()
    
    const fullConfig: TTSConfig = {
      rate: config?.rate ?? speechSpeed,
      pitch: config?.pitch ?? 0.9,  // Slightly lower pitch
      volume: config?.volume ?? 0.3,  // Default to 30% volume (much quieter)
      voice: config?.voice,
    }
