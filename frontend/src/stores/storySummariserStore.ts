...
const API_BASE = '/story-summariser';
  ...
  (set, get) => ({
    ...
    generateFromText: async (text, title, level = 'moderate') => {
      set({ isGenerating: true, errorMessage: null });
        ...
        try {
          const response = await apiClient.post<StorySummary>(`${API_BASE}/generate`, {
            text,
            title
          });

          // Use the backend response directly (has relevant illustrations)
          if (response.success && response.data) {
            const storySummary: StorySummary = {
              title: response.data.title || title || 'Story Summary',
              panels: response.data.panels || [],
              panelCount: response.data.panelCount || 0,
              originalLength: response.data.originalLength || 0,
          ...
          // Fallback: create panels from summary text
    ...
    playNarration: async (panelIndex: number, onStart?, onEnd?) => {
      ...
      if ('speechSynthesis' in window) {
        ...
        utterance.volume = 0.5;  // Reduce volume
